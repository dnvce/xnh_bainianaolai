...
class TestAddress:
    ...
    def test_add_address(self, args):
        ...
        if toast is None:
            # "张三  18888888888"
            assert self.page.address_list.get_default_receipt_name_text() == "%s  %s" % (
                name, phone), "保存成功，默认的姓名和电话与输入的不符"
        else:
            assert self.page.edit_address.is_toast_exist(toast), "保存不成功，toast内容和预期不符"

    # 如果地址不存在则创建,该方法不使用参数化,因为想要修改,必须要添加一个成功的地址
