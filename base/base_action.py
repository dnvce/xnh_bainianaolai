...
class BaseAction:
    ...
    # 显示等待获取一个元素
    def find_element(self, feature, timeout=30, poll=1.0):
        by = feature[0]
        value = feature[1]
        return WebDriverWait(self.driver, timeout, poll).until(lambda x: x.find_element(by, value))

    # 显示等待获取一组元素
    def find_elements(self, feature, timeout=30, poll=1.0):
        by = feature[0]
        value = feature[1]
    ...
    def is_feature_exist(self, feature):
        try:
            self.find_element(feature, 3, 0.1)
            return True
        except TimeoutException:
            return False